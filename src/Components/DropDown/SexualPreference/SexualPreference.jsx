...
// STYLES
import { Wrapper } from "./SexualPreference.style";
...
function SexualPreference(props) {
	let [sexualPreference, setSexualPreference] = useState([]),
	...
	async function getSexualPreference(pagination = paginationInfo) {
		try {
			setLoading(true);
			let response = await new Api().post("sexual-preference/list", {
				data: {
			...
			setSexualPreference(response.data.rows);
	...
	function onSuccess(type, data) {
		...
		if (type === MODAL_TYPES.EDIT) {
			let newList = [...sexualPreference];

			newList.forEach((item, index) => {
				if (item.id === data.id) newList[index] = { ...data };
			});
			setSexualPreference(newList);
		} else {
			getSexualPreference();
		}
	}
	...
	async function onDelete(id) {
		setLoading(true);
		await new Api().delete("sexual-preference/" + id);
		getSexualPreference();
	}
	...
	function onTableChange(...rest) {
		...
		getSexualPreference(newPaginationInfo);
		...
		<Wrapper>
			<Card spacing={24}>
				<div className="title-wrapper flex">
					<PageTitle>Sexual Preference</PageTitle>
